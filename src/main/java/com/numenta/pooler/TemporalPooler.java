...
public class TemporalPooler {
		...
		for (int c = 0; c < SpatialPooler.AMMOUNT_OF_COLLUMNS; c++)
			for (int i = 0; i < Column.CELLS_PER_COLUMN; i++) {
				...
				if (cells[c][i][Cell.NOW].hasLearnState()) {
					...
					if(cell.getSegmentUpdateList()!=null){
	 ...
	 * activity od a segment as most connected synapses
	 * 
	 * @param c
	 * @param i
	 * @param time
	...
	// als het aantal connected synapses gelijk is, kijk dan naar het totaal
	// aantal synapses.
	...
	private boolean segmentActive(Segment segment, int time, boolean state) {
		...
		for (LateralSynapse synapse : synapses) {
			...
			if (synapse.isActive()
					&& cells[synapse.getColumnIndex()][synapse.getCellIndex()][time]
							.hasLearnState() == state) {
				ammountConnected++;
			}
		}
